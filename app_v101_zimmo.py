...
# =========================================================
# V10.1 — ZIMMO / MARKETVIEW
# =========================================================
def zimmo_feed_ready():
    return bool(secret("ZIMMO_FEED_URL", ""))

@st.cache_data(ttl=900, show_spinner=False)
def fetch_zimmo_feed(url):
    headers = {
        "User-Agent": "ChasseurPepitesV10.1/1.0",
        "Accept": "application/json,text/csv,text/plain,*/*",
    }
    token = str(secret("ZIMMO_API_TOKEN", ""))
    header_name = str(secret("ZIMMO_AUTH_HEADER", "Authorization"))
    if token:
    ...
    if "json" in ctype or url.lower().endswith(".json"):
        ...
        if isinstance(payload, dict):
            for key in ("results","data","items","properties","listings","annonces"):
                if isinstance(payload.get(key), list):
                    payload = payload[key]
                    break
        if not isinstance(payload, list):
            raise RuntimeError("Le flux JSON Zimmo ne contient pas une liste reconnue.")
...
def normalize_zimmo_frame(df):
    def get_val(row, aliases, default=None):
        cols = {norm(c): c for c in row.index}
    ...
    for i, row in df.iterrows():
        title = str(get_val(row, ["titre","title","name","description","bien"], f"Zimmo {i+1}"))
        city = str(get_val(row, ["ville","city","commune","gemeente","locality"], ""))
        price = to_num(get_val(row, ["prix","price","asking price","vraagprijs"], 0), 0)
        surface = to_num(get_val(row, ["surface habitable","surface","living area","woonoppervlakte","bewoonbare oppervlakte"], 0), 0)
        land = to_num(get_val(row, ["terrain","plot area","land area","grond","perceel"], 0), 0)
        beds = int(to_num(get_val(row, ["chambres","bedrooms","slaapkamers"], 0), 0))
        peb = str(get_val(row, ["peb","epc"], "Inconnu"))
        url = str(get_val(row, ["url","link","lien","zimmo url"], ""))
        zid = str(get_val(row, ["id","listing id","property id","reference","ref"], "")).strip()

        if price <= 0:
            continue
        if not zid:
            zid = str(abs(hash(f"{title}|{city}|{price}|{url}")))

        stat = statbel_reference(city) if st.session_state.statbel_loaded and city else None
...
# HEADER
# =========================================================
st.title("🏠 Chasseur de Pépites — V10.1 Zimmo")
...
with tab_hunt:
    ...
    if zimmo_feed_ready():
        s1.success("✅ Zimmo prêt")
    else:
        s1.warning("⚠️ Zimmo non configuré")

    if realo_ready():
        s2.success("✅ Realo connecté")
    else:
        s2.info("Realo en attente")

    if st.session_state.statbel_loaded:
        s3.success("✅ Statbel chargé")
    else:
        s3.warning("⚠️ Statbel absent")

    if not zimmo_feed_ready() and not realo_ready():
        st.info(
            "Aucune source automatique n'est encore connectée. "
            "Tu peux commencer immédiatement avec un export Zimmo/Marketview CSV ou JSON ci-dessous."
        )
    ...
    st.markdown("### Import Zimmo immédiat")
    zimmo_upload = st.file_uploader(
        "Déposer un export Zimmo/Marketview CSV ou JSON",
        type=["csv","json"],
        key="zimmo_marketview_upload"
    )

    if zimmo_upload is not None:
        try:
            if zimmo_upload.name.lower().endswith(".json"):
                payload = json.load(zimmo_upload)
                if isinstance(payload, dict):
                    for key in ("results","data","items","properties","listings","annonces"):
                        if isinstance(payload.get(key), list):
                            payload = payload[key]
                            break
                zdf = pd.DataFrame(payload)
            else:
                raw = zimmo_upload.getvalue()
                zdf = pd.read_csv(BytesIO(raw), sep=None, engine="python")

            zprops = normalize_zimmo_frame(zdf)
            st.write(f"**{len(zprops)} annonce(s) Zimmo exploitable(s)**")

            if zprops and st.button("Ajouter l'export Zimmo au chasseur", key="add_zimmo_export"):
                ...
                for p in zprops:
                    if p["id"] not in known:
                        ...
                        known.add(p["id"])
                        added += 1
                st.success(f"{added} nouvelle(s) annonce(s) Zimmo ajoutée(s).")
                st.rerun()
        except Exception as e:
            st.error(f"Export Zimmo illisible : {e}")
    ...
    if st.button("🚀 LANCER LA CHASSE", type="primary", use_container_width=True):
        ...
        if zimmo_feed_ready():
            used_source = True
            try:
                with st.spinner("Chargement Zimmo / Marketview…"):
                    zdf = fetch_zimmo_feed(str(secret("ZIMMO_FEED_URL")))
                    zprops = normalize_zimmo_frame(zdf)
                    known = {p["id"] for p in st.session_state.hunter_rows}
                    for p in zprops:
                        if p["prix"] <= hunt_budget and p["id"] not in known:
                            st.session_state.hunter_rows.append(p)
                            known.add(p["id"])
                            total_added += 1
                st.success(f"Zimmo : {len(zprops)} annonce(s) reçue(s).")
            except Exception as e:
                st.error(f"Zimmo : {e}")
        ...
        if realo_ready():
            ...
            try:
                ...
                params = {"q": st.session_state.center_city, "price_max": int(hunt_budget), "radius": int(hunt_radius)}
                with st.spinner("Recherche Realo…"):
        ...
        if not used_source:
            st.error("Aucune source automatique configurée. Utilise l'import Zimmo ci-dessus ou configure ZIMMO_FEED_URL.")
        else:
            st.success(f"Chasse terminée : {total_added} nouvelle(s) annonce(s) ajoutée(s).")
