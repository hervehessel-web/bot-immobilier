...
# HEADER
# =========================================================
st.title("🏠 Chasseur de Pépites — V9")
...
# ---------------------------------------------------------
# 1 HUNT — AUTHORIZED / USER-SUPPLIED SOURCES
# ---------------------------------------------------------
with tab_hunt:
    st.subheader("🔎 Chasser les opportunités")
    st.write(
        "Importe un fichier CSV/JSON provenant d'une source autorisée, d'un export personnel "
        "ou d'un partenaire. V9 normalise les annonces, retire les doublons et classe les meilleures."
    )

    c1,c2,c3,c4 = st.columns(4)
    hunt_budget = c1.number_input("Prix max (€)", 0, 2_000_000, int(st.session_state.budget_max), 5000, key="hunt_budget")
    hunt_works = c2.number_input("Travaux max (€)", 0, 1_000_000, int(st.session_state.works_max), 5000, key="hunt_works")
    hunt_profit = c3.number_input("Bénéfice min (€)", 0, 500_000, int(st.session_state.target_profit), 5000, key="hunt_profit")
    hunt_score = c4.slider("Score min", 0, 100, 60, 5, key="hunt_score")

    st.markdown("### Import en masse")
    st.caption(
        "Colonnes reconnues automatiquement : titre/name, ville/city/commune, prix/price, "
        "surface, terrain, chambres, peb, travaux/works, revente/resale, url/link."
    )

    bulk = st.file_uploader("CSV ou JSON d'annonces", type=["csv","json"], key="v9_bulk")

    def pick_value(row, names, default=None):
        mapping = {norm(k): k for k in row.index}
        for n in names:
            nn = norm(n)
            for nk, original in mapping.items():
                if nn == nk or nn in nk:
    ...
    def normalize_bulk(df):
        out = []
        for i, row in df.iterrows():
            title = str(pick_value(row, ["titre","title","name","nom"], f"Annonce {i+1}"))
            city = str(pick_value(row, ["ville","city","commune","gemeente"], st.session_state.center_city))
            price = to_num(pick_value(row, ["prix","price","vraagprijs"], 0))
            surface = to_num(pick_value(row, ["surface habitable","surface","living area","woonoppervlakte"], 0))
            land = to_num(pick_value(row, ["terrain","land","parcelle","grond"], 0))
            beds = int(to_num(pick_value(row, ["chambres","bedrooms","slaapkamers"], 0)))
            peb = str(pick_value(row, ["peb","epc"], "Inconnu"))
            works = to_num(pick_value(row, ["travaux","works","renovation"], 0))
            resale = to_num(pick_value(row, ["revente","resale","valeur apres travaux","after renovation"], 0))
            url = str(pick_value(row, ["url","link","lien"], ""))
            ...
            rid_base = url.strip() or f"{title}|{city}|{price}|{surface}"
            rid = "H-" + str(abs(hash(rid_base)))
            ...
            prop = {
                ...
                "ville": city,
                "prix": price,
                "surface": surface if surface > 0 else 1.0,
                "terrain": land,
                "chambres": beds,
                "peb": peb,
                "travaux": works,
                "travaux_confiance": "Faible" if works <= 0 else "Moyenne",
                "revente": resale,
                "revente_confiance": "Faible" if resale <= 0 else "Moyenne",
                "comparables_count": 0,
                "url": url,
                "notes": "Import V9",
    ...
    if bulk is not None:
        try:
            if bulk.name.lower().endswith(".json"):
                raw = json.load(bulk)
                if isinstance(raw, dict):
                    raw = raw.get("annonces", raw.get("properties", raw.get("data", [])))
                df_bulk = pd.DataFrame(raw)
            else:
                # Auto-detect separator
                content = bulk.getvalue()
                try:
                    df_bulk = pd.read_csv(BytesIO(content), sep=None, engine="python")
                except Exception:
                    df_bulk = pd.read_csv(BytesIO(content), sep=";")

            st.write(f"**{len(df_bulk)} ligne(s) détectée(s).**")
            normalized = normalize_bulk(df_bulk)
            st.write(f"**{len(normalized)} annonce(s) exploitable(s).**")

            if normalized:
                if st.button("Analyser et dédupliquer", type="primary", key="v9_analyze_bulk"):
                    existing = {p["id"] for p in st.session_state.hunter_rows}
                    added = 0
                    for p in normalized:
                        if p["id"] not in existing:
                            ...
                            existing.add(p["id"])
                            added += 1
                    st.success(f"{added} nouvelle(s) annonce(s) ajoutée(s) au chasseur.")
                    st.rerun()
        except Exception as e:
            st.error(f"Import impossible : {e}")

    st.markdown("### Résultats du chasseur")
    hunter = st.session_state.hunter_rows
    if not hunter:
        st.info("Aucune annonce importée pour l'instant.")
    else:
        ranked = []
        for p in hunter:
            # Incomplete rows cannot be scored as real opportunities.
            complete = p["travaux"] > 0 and p["revente"] > 0
            if complete:
                f = financials(p)
                sc = opportunity_score(p)
                dec, _ = decision(p)
                maxp = solve_max_price(p)
                profit = f["benefice"]
                margin = f["marge"]
            else:
                sc = quality_score(p)[0] * 0.4
                dec = "À COMPLÉTER"
                maxp = 0
                profit = 0
                margin = 0

            if p["prix"] <= hunt_budget and p["travaux"] <= hunt_works:
                ranked.append({
                    "Décision": dec,
                    "Score": round(sc,1),
                    "Bien": p["titre"],
        ...
        if rankdf.empty:
            st.warning("Aucune annonce ne respecte les filtres actuels.")
        else:
            # Apply financial thresholds only when calculable.
            rankdf = rankdf[
                ((rankdf["Bénéfice"] >= hunt_profit) & (rankdf["Score"] >= hunt_score))
                | (rankdf["Décision"] == "À COMPLÉTER")
            ]
            rankdf = rankdf.sort_values(["Score","Bénéfice"], ascending=[False,False])

            st.dataframe(
                rankdf.drop(columns=["ID"]),
            ...
            if not rankdf.empty:
                labels = {
                    ...
                    for _,r in rankdf.iterrows()
                }
                chosen = st.selectbox("Sélectionner une annonce", list(labels.keys()), key="v9_pick")
                chosen_id = labels[chosen]
                chosen_prop = next(p for p in hunter if p["id"] == chosen_id)

                if st.button("⭐ Ajouter à MES BIENS", type="primary", key="v9_add_selected"):
                    add_or_replace(chosen_prop)
                    st.session_state.selected_id = chosen_prop["id"]
                    st.success("Annonce ajoutée à MES BIENS.")

    with st.expander("Comment automatiser davantage ?"):
        st.write(
            "V9 est prête à recevoir une API partenaire comme Realo dès que les clés sont disponibles. "
            "Elle n'effectue pas de scraping agressif ni de contournement des protections des portails."
        )
...
st.divider()
st.caption(
    "V9 sépare clairement deux notions : la rentabilité estimée et la qualité des données. "
