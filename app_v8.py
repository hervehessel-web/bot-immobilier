...
# HEADER
# =========================================================
st.title("🏠 Chasseur de Pépites — V8")
...
# FOUR MAIN SCREENS
# =========================================================
tab_find, tab_analyze, tab_decide, tab_my = st.tabs([
    "🏠 1. AJOUTER",
    "🔍 2. ANALYSER",
    "🚦 3. DÉCIDER",
    "⭐ 4. MES BIENS",
...
with tab_find:
    ...
    st.write("Entre seulement les informations utiles. Le bot affiche ensuite la fiabilité de l'analyse.")

    with st.form("add_property", clear_on_submit=False):
        url = st.text_input("Lien de l'annonce")
        c1,c2,c3 = st.columns(3)
        title = c1.text_input("Nom / référence", "Maison à rénover")
        city = c2.text_input("Commune", st.session_state.center_city)
        price = c3.number_input("Prix demandé (€)", 0, 2_000_000, 100000, 5000)

        c1,c2,c3,c4 = st.columns(4)
        surface = c1.number_input("Surface habitable (m²)", 1.0, 3000.0, 150.0, 5.0)
        land = c2.number_input("Terrain (m²)", 0.0, 100000.0, 800.0, 50.0)
        beds = c3.number_input("Chambres", 0, 30, 3, 1)
        peb = c4.selectbox("PEB", ["A++","A+","A","B","C","D","E","F","G","Inconnu"], index=9)
...
st.divider()
st.caption(
    "V8 sépare clairement deux notions : la rentabilité estimée et la qualité des données. "
