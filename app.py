...
D = dict(budget=150000, radius=80, works_max=120000, profit=40000, margin=20.0,
         rate=3.8, months=12, finance=100, reg=12.5, notary=1.5,
         resale=3.0, contingency=10.0, holding=350, city="Houffalize")
for k,v in D.items():
    st.session_state.setdefault(k,v)

# Migration depuis la première V4 : si une ancienne clé "works" existe,
# on la recopie vers "works_max" uniquement si nécessaire.
if "works_max" not in st.session_state:
    st.session_state["works_max"] = st.session_state.get("works", 120000)
