...
with st.sidebar:
    ...
    st.session_state.works = st.number_input("Travaux max (€)",0,1_000_000,int(st.session_state.works),5000)
...
for _,r in df.iterrows():
    ...
    score+=10 if float(r.travaux)<=st.session_state.works else 0
...
qualified=res[(res.prix<=st.session_state.budget)&(res.distance_km<=st.session_state.radius)&
              (res.travaux<=st.session_state.works)&(res.benefice>=st.session_state.profit)&
...
with tworks:
    labels=(res.titre.astype(str)+" — "+res.ville.astype(str)).tolist()
    pick=st.selectbox("Bien",labels,key="works")
    r=res.iloc[labels.index(pick)]
